---
title: "Species Distribution Modelling in R: A Case Study of *Bidens bipinnata* on Fogo Island"
...
This tutorial presents the workflow for building a Species Distribution Model (SDM) for *Bidens bipinnata*, the most abundant introduced species on Fogo Island, Cabo Verde (West Africa). 

The tutorial draws inspiration from [Oliver's (2024)](https://jcoliver.github.io/learn-r/011-species-distribution-models) work. However, we have made significant changes to the environmental data processing as these data have different resolutions and coordinate reference systems (CRS) in the present study.

We used **R Markdown**, a simple formatting syntax for authoring HTML, MS Word, and PDF documents (see <http://rmarkdown.rstudio.com>). So, when you open the R Markdown file and click the **Knit** button, a document will be generated. It includes both content and the output of any embedded R code chunks within the document.
...
```{r}
obs_data <- read.csv(file = "data_occ/Bidens_bipinnata.csv")
head(obs_data)
```
...
data_path <- "data_env/climate/wc2.1_country" 
...
```{r}
landcov <- rast("data_env/landcover_model2.tif")
plot(landcov)
```

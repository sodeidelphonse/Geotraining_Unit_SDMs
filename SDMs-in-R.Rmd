...
For simplicity, we will consider only three bioclimatic variables: annual temperature (bio1), temperature seasonality (bio4) and annual precipitation (bio12). 
...
Since all the environmental variables should have the same projection system and resolution, we project the landcover into long/lat system. However, we should be aware that it is not a good practice to project raster layers due to the lack of precision associated with raster layers transformation. 
...
We can now merge the three environmental variables into a raster stack and visualize them.
...
we then create a set of 500 background (aka pseudo-absence) points at random, and add these to our data. For a large study region with a high resolution environmental data, think about trying 1000 or even 10,000 points.
